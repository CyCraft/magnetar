...
test('fetch (document) if unfetched', async () => {
  /// get resolves once all stores have given a response with data
  const { trainerModule } = createMagnetarInstance()
  assert.deepEqual(trainerModule.data, { name: 'Luca', age: 10 })
  try {
  ...
  // because even though the the cache state was already there, we had never executed `fetch` or `stream` once in this session
  assert.deepEqual(trainerModule.data, { name: 'Luca', age: 10, dream: 'job' })
})
...
test('fetch (collection) startAfter', async () => {
  const { pokedexModule } = createMagnetarInstance()
  try {
